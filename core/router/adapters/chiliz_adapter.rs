// Chiliz Chain Adapter - Sports & entertainment blockchain
// Production-ready implementation with full EVM compatibility

use async_trait::async_trait;
use ethers::prelude::*;
use ethers::providers::{Provider, Ws, Http, Middleware};
use ethers::types::{Address, U256, TransactionRequest, Bytes, BlockNumber, H256};
...
impl ChilizAdapter {
    pub async fn new(
        ...
        let chain_id = match network {
            ChilizNetwork::Mainnet => 88888,
            ChilizNetwork::Spicy => 88882, // Testnet
        };
    ...
    async fn estimate_chiliz_gas(&self, tx: &TransactionRequest) -> Result<(U256, U256), RouterError> {
        ...
        // Add 15% buffer for gas price volatility
        let adjusted_gas_price = gas_price * 115 / 100;
...
pub enum ChilizNetwork {
    Mainnet,
    Spicy, // Testnet
}

#[async_trait]
impl ChainAdapter for ChilizAdapter {
    ...
    async fn submit_transaction(&self, tx_data: &[u8]) -> Result<String, RouterError> {
        ...
        let (gas_limit, gas_price) = self.estimate_chiliz_gas(&tx).await?;
    ...
    async fn query_balance(&self, address: &str, asset: &str) -> Result<u64, RouterError> {
        ...
        if asset.is_empty() || asset.to_uppercase() == "CHZ" {
            // Native CHZ balance
            ...
            // Query fan token or other ERC20 balance
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_chiliz_connection() {
        let translator = Arc::new(IntentTranslator::new());
        let adapter = ChilizAdapter::new(
            "wss://rpc.ankr.com/chiliz/ws".to_string(),
            "https://rpc.ankr.com/chiliz".to_string(),
            ChilizNetwork::Mainnet,
            translator,
        ).await;
