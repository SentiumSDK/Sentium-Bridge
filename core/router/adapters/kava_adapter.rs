// REAL Kava Adapter - Cosmos-based DeFi platform
...
#[async_trait]
impl super::chain_adapter::ChainAdapter for RealKavaAdapter {
    fn chain_name(&self) -> &str { &self.chain_name }
    fn chain_id(&self) -> &str { "kava-mainnet" }
    ...
    async fn query_balance(&self, address: &str, _asset: &str) -> Result<u64, RouterError> {
        ...
        if let Some(balances) = result["balances"].as_array() {
            for balance in balances {
                if balance["denom"].as_str() == Some("ukava") {
