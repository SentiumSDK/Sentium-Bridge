// REAL BounceBit Adapter - Production-ready implementation
// BounceBit is a Bitcoin restaking infrastructure with EVM compatibility
...
pub struct RealBounceBitAdapter {
    chain_name: String,
    chain_id: u64,
...
impl RealBounceBitAdapter {
    pub async fn new(
        ws_url: String,
        http_url: String,
        chain_id: u64, // 6001 for mainnet, 6000 for testnet
        translator: Arc<IntentTranslator>,
        ...
        Ok(Self {
            chain_name: "bouncebit".to_string(),
            chain_id,
...
#[async_trait]
impl super::chain_adapter::ChainAdapter for RealBounceBitAdapter {
    fn chain_name(&self) -> &str {
        &self.chain_name
    }
    
    fn chain_id(&self) -> &str {
        if self.chain_id == 6001 {
            "bouncebit-mainnet"
        } else if self.chain_id == 6000 {
            "bouncebit-testnet"
        } else {
            "bouncebit-unknown"
        }
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_bouncebit_connection() {
        let translator = Arc::new(IntentTranslator::new());
        let adapter = RealBounceBitAdapter::new(
            "wss://fullnode-testnet.bouncebitapi.com/".to_string(),
            "https://fullnode-testnet.bouncebitapi.com/".to_string(),
            6000,
            translator,
        ).await;
